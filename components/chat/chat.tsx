...
export default function Chat() {
  ...
  // 1. SECURITY: Private route (incoming user is dropped)
  useEffect(() => {
    if (!authLoading && !user) {
      router.push("/");
    }
  ...
  // Prepare the audio object for the notification sound
  useEffect(() => {
  ...
  // 3. REALTIME: New messages and the “typing...” listener
  useEffect(() => {
    if (!supabase || !user) return;

    const client = supabase;

    // Message listener (Postgres changes)
    const msgChannel = client
      ...
      .on(
        ...
        (payload) => {
          ...
          // Only voice and browser notification in another person's message
          if (!isMe) {
            // Sound effect
            ...
            if (
              ...
              new Notification("Yeni mesaj", {
                body: `${isMe ? username : "Usuário"}: ${msg.content}`,
              });
            }
          }
  ...
  // Send message (add to list immediately, keep Realtime running)
  const handleSend = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Optimistic update for the message to appear immediately
    if (data) {
  ...
  // user online (Supabase Realtime Presence)
  useEffect(() => {
    if (!supabase || !user || !username) return;

    const client = supabase;

    // We don't provide a custom key - Supabase generates a unique UUID for each connection (important for tab switching/refreshing)
        ...
        <div className="w-full flex flex-col flex-1 lg:max-h-[calc(100dvh-2rem)] min-w-0">
          ...
          <div className="glass rounded-t-2xl p-3 sm:p-6 border border-border flex-shrink-0">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center justify-between gap-3">
                ...
                {/* Mobile: button to open the online list */}
                <button
                  type="button"
          ...
          <div className="glass border-x border-border p-2 sm:p-4 flex-1 overflow-y-auto custom-scrollbar min-h-[300px]">
            <div className="flex flex-col gap-4">
              ...
              {messages.map((msg) => {
                const isMe = msg.user_id === user.id; // Mesaj mənəmi aiddir?
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {/* Avatar - only in someone else's message, or if you want, in both */}
                      <div
                      ...
                      >
                        <span className="text-white text-[10px] font-bold">
                          {(isMe ? username : "U").charAt(0).toUpperCase()}
                        </span>
                      </div>
